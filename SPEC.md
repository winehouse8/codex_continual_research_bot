...
`같은 topic을 n번 연구했을 때, 시스템이 더 많은 문장을 쌓았느냐가 아니라 더 좋은 hypothesis state에 도달했느냐`
...
### 3.2 Codex-Centered User Experience
...
- 반복 실행될수록 접근 전략이 다양해지고 정보가치가 증가한다.
...
핵심은 새 정보 수집뿐 아니라 기존 belief state의 재검토다.
...
- primary execution path: `user-owned OpenAI Codex Max plan`

### 16.2 Why This Stack

- Python은 orchestration과 backend iteration 속도가 빠르다.
- Agents SDK는 tool-driven research loop 구성에 적합하다.
