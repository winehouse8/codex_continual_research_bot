...
- `semantic validator`: evidence id uniqueness, referenced artifact existence, temporal scope presence, hypothesis ids consistency
- `policy validator`: forbidden field, unsupported graph mutation, unresolved citation placeholder 금지

### 9.4 Repair Prompt Shape

repair prompt는 아래처럼 최소화한다.
...
오직 final candidate와 violation set만 제공해 drift를 줄인다.
